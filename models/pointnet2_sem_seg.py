...
class get_model(nn.Module):
    ...
    def forward(self, xyz):
        ...
        x = self.conv2(x) #(B,num_classes,2048)
        x = F.log_softmax(x, dim=1)
        x = x.permute(0, 2, 1)#(B,2048,num_classes)
        return x, l4_points#(B,2048,num_classes) (B,512,16)
...
if __name__ == '__main__':
    import  torch
    model = get_model(13)
    xyz = torch.rand(6, 9, 2048) # input shape: (Batch_size,Channel,Num_of_pts)
    (model(xyz))# output shape: (Batch_size,num_classes,Num_of_pts)